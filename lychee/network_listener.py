...
class NetworkFileListener(object):
	...
	def detect_local_ips(self):
		'''Determine all of the local ip addresses for this machine so we can flag 
		Traffic as inbound or outbound'''
	...
	def _handle_packet(self, pkt):	   
		...
		if tcp.data.startswith('GET') or tcp.data.startswith('POST'):
			if is_outbound:
				# ignore inbound http request

				logging.debug('Detected an %s HTTP Request from %s to %s' % (direction, ipaddr_string(ip.src), ipaddr_string(ip.dst)))
